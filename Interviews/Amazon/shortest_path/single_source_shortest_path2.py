...
def djikstra(start="1"):
    ...
    found = {i: False for i in weighted_graph.keys()}
    distances = {i: math.inf for i in weighted_graph.keys()}
    ...
    for i in range(1,n-1):
        u = get_next_non_visited_min(vertices,visited=found, distances=distances)
        found[u] = True
        for w in range(1, n + 1):
            w = str(w)
            if not found[str(w)]:
                if (u,w) in edges:
                    if distances[u] + edges[(u,w)] < distances[w]:
                        distances[w] = distances[u] + edges[(u,w)]
    pprint("distances")
    pprint(distances)
    pprint("found")
    pprint(found)
